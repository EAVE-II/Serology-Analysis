...
data.pc <- readRDS("/conf/EAVE/GPanalysis/data/temp/fail_to_mount_pc_Jan10_v1.rds")
data.bd <- readRDS("/conf/EAVE/GPanalysis/data/temp/fail_to_mount_bd_Jan10_v1.rds")
print (nrow(data.pc))
print (nrow(data.bd))
```
...
```{r}
output_folder <- file.path("results", "VACCINE_FIX")# Sys.Date())
...
```{r}
pc.modelDE.df <- eavehelpers::serology_outcome_analysis.create_hosp_death_dataframe(data.pc,eave.data) %>% code_vars() %>% filter(ageYear!='0-19')
bd.modelDE.df <- eavehelpers::serology_outcome_analysis.create_hosp_death_dataframe(data.bd,eave.data) %>% code_vars() %>% filter(ageYear!='0-19')
```
