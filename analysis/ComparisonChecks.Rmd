...
pc.modelA.v3 <- readRDS('/home/calumm09/SerologyAnalysis/analysis/results/QCOVID_FIX/modelA_pc_gamFit.rds')
...
```{r}
pc.modelA.v1.or <- eavehelpers::get_or_from_gam(pc.modelA.v1) 
pc.modelA.v2.or <- eavehelpers::get_or_from_gam(pc.modelA.v2) 
pc.modelA.v3.or <- eavehelpers::get_or_from_gam(pc.modelA.v3) 
pc.modelA.v4.or <- eavehelpers::get_or_from_gam(pc.modelA.v4) 

bd.modelA.v1.or <- eavehelpers::get_or_from_gam(bd.modelA.v1) 
bd.modelA.v2.or <- eavehelpers::get_or_from_gam(bd.modelA.v2) 
bd.modelA.v3.or <- eavehelpers::get_or_from_gam(bd.modelA.v3) 
bd.modelA.v4.or <- eavehelpers::get_or_from_gam(bd.modelA.v4) 
...
```{r}
pc.modelD.v1 <- readRDS('/home/calumm09/SerologyAnalysis/analysis/results/Nominal/modelD_cox_HR.rds')
pc.modelD.v2 <- readRDS('/home/calumm09/SerologyAnalysis/analysis/results/VACCINE_FIX/modelD_cox_HR.rds')
pc.modelD.v2
```


```{r}
temp <- pc.modelD.v1 %>% mutate(Nominal=estimate,LCL=conf.low,UCL=conf.high,label=name, dataset='Nominal') %>%
           rbind(
             pc.modelD.v2 %>% mutate(Nominal=estimate,LCL=conf.low,UCL=conf.high,label=name, dataset='Fix')
           )
