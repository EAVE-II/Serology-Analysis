...
for (var in vars){
  ...
  rows <- col1 %>% left_join(col2) %>%  left_join(col3) %>%  left_join(col4) %>% 
          #rename(label=!!sym(var)) %>% mutate(var=eavehelpers::get_label(as.character(var)),.before=label)
          rename(label=!!sym(var)) %>% mutate(var2=eavehelpers::get_label(as.character(var)),.before=label) %>%
          mutate(var = ifelse(is.na(var2),var,var2),.before=label) %>% select(-var2)
...
tab %>% group_by(var) %>% filter(!(label=='No')) %>% ungroup %>% mutate_all(~replace_na(as.character(.),'-')) %>% 
