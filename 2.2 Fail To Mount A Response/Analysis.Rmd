...
```{r}
df_ana %>% filter(outcome==1) %>% summarise(across(where(is.numeric), ~ sum(.x, na.rm = TRUE))) 
```

Peform the GLM
```{r}
glmFit <- glm(outcome ~ days_since_vac2  + n_risk_gps_int + age + sex + product + Q_BMI + Q_DIAG_DIABETES_1 + Q_DIAG_RESP_CANCER + Q_DIAG_ASTHMA + Q_DIAG_BLOOD_CANCER + Q_DIAG_CHD + Q_DIAG_COPD + Q_DIAG_EPILEPSY + Q_DIAG_FRACTURE + Q_DIAG_IMMU + Q_DIAG_NEURO + Q_DIAG_PARKINSONS + Q_DIAG_PARKINSONS + Q_DIAG_PULM_RARE + Q_DIAG_PVD + Q_DIAG_RA_SLE + Q_DIAG_SEV_MENT_ILL + Q_DIAG_STROKE + Q_DIAG_VTE
                  ,family=poisson,data=df_ana)
summary(glmFit)
```
